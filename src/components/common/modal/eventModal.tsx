...
export function SearchWinnerDialog(props: SearchWinnerDialog) {
  ...
  const { data, isFetching } = trpc.eventTicket.getEventTicketCustomer.useQuery(
    ...
    {
      enabled: debouncedTicketNumber > 0,
    },
  );
  ...
  return (
    <Dialog open={props?.open} onOpenChange={props.openChangeHandler}>
      <DialogContent className="max-w-[675px]   overflow-x-scroll no-scrollbar">
        <DialogHeader>
          <DialogTitle>Select Winner</DialogTitle>
              ...
              <Table>
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">Ticket No.</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  <TableRow>
